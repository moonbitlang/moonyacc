...
- In `array` mode, the parsing function has the signature `(Array[(Token, Int, Int)]) -> R!ParseError`, where the input is an array of tokens with position information. To support location information, the tokens array must have at least one element.
